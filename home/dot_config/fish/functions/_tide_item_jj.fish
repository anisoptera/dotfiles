...
# Local changes:
#   - segment icon comes from $tide_jj_icon instead of $tide_git_icon
#   - re-assert the segment background after the full resets (\e[0m) that
#     `jj --color always` emits, so the whole segment keeps tide's bg color
function _tide_item_jj
    ...
    set -l reset \e'[0m'
    set -l segment_bg (set_color -b $tide_jj_bg_color)
    set wc_info (string replace -a -- $reset $reset$segment_bg $wc_info)
    ...
    _tide_print_item jj $_tide_location_color$tide_jj_icon' ' (echo -ns $wc_info
