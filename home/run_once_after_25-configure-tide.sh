#!/bin/sh
# Replays the saved Tide prompt configuration (captured from the primary
# machine with `set -U --names | string match -r ^tide_`), then re-enables
# the jj-aware vcs item. Update by re-dumping after any `tide configure`.
set -e
fish=$(command -v fish || true)
[ -z "$fish" ] && [ -x /opt/homebrew/bin/fish ] && fish=/opt/homebrew/bin/fish
[ -z "$fish" ] && { echo "fish not found; skipping tide setup" >&2; exit 0; }
"$fish" <<'EOF'
...
set -U tide_left_prompt_items vi_mode os pwd vcs newline
